...
/* Rename to avoid collision with Windows GetUserName API */
bool LBGetUserName(char* name, u_int32_t* bytes);
char* itoa(int value, char* string, int radix);
char* Ltoa(int32_t value, char* string, int radix);
char* qmrealpath(char* inpath, char* outpath);
/* On Windows the Sleep function is provided by the Win32 API (in <windows.h>).
   Avoid redeclaring it to prevent C2373 redefinition errors. Only declare
   Sleep here for non-Windows builds where the helper is implemented. */
#ifndef _WIN32
